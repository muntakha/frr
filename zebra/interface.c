...
static void
link_param_cmd_set_uint16 (struct interface *ifp, uint16_t *field,
		uint32_t type, uint16_t value)
{
	/* Update field as needed */
	if (IS_PARAM_UNSET(ifp->link_params, type) || *field != value)
	{
...
/*mes modifs*/
/* srlg */
DEFUN (link_params_srlg,
		link_params_srlg_cmd,
		"srlg (0-4294967295)",
		"Shared risk link group\n"
		"Srlg value in decimal\n")
{
	int idx_number = 1;
	VTY_DECLVAR_CONTEXT (interface, ifp);
	struct if_link_params *iflp = if_link_params_get (ifp);
	...
	VTY_GET_ULONG("SRLG", srlg, argv[idx_number]->arg);

	/* Update srlg */
	link_param_cmd_set_uint32 (ifp, &iflp->srlg, LP_SRLG, srlg);
	/* force protocols to update LINK STATE due to parameters change */
	if (if_is_operative (ifp))
		zebra_interface_parameters_update (ifp);
...
DEFUN (no_link_params_srlg,
		no_link_params_srlg_cmd,
		"no srlg",
		NO_STR
		"Disable srlg on this interface\n")
{
	VTY_DECLVAR_CONTEXT (interface, ifp);

	/* Unset TE srlg */
	link_param_cmd_unset(ifp, LP_SRLG);
	/* force protocols to update LINK STATE due to parameters change */
	if (if_is_operative (ifp))
		zebra_interface_parameters_update (ifp);
	return CMD_SUCCESS;
		...
		"Disable switching capability on this interface\n")
{
	...
	/* Unset TE iscd */
	link_param_cmd_unset(ifp, LP_ISCD);
		...
		"Fixed grids at 100 GHz or 50 GHz\n")
{
	...
	VTY_DECLVAR_CONTEXT (interface, ifp);
	struct if_link_params *iflp = if_link_params_get (ifp);
	strcpy(grid_fixe,"fixed_50GHz");
	strcpy(grid_fixe1,"fixed_100GHz_as_50");

	/* check if the grid is fixed at 50GHz*/
	...
	{
		vty_out (vty, "fixed grid: fscanf: %s%s", safe_strerror (errno),
				VTY_NEWLINE);
		return CMD_WARNING;
	}
		...
		"Disable fixed grid on this interface\n")
{
	...
	/* Unset TE srlg */
	link_param_cmd_unset(ifp, LP_ISCD_SCSI);
	/* force protocols to update LINK STATE due to parameters change */
	if (if_is_operative (ifp))
		zebra_interface_parameters_update (ifp);
	return CMD_SUCCESS;
